...
export const WithdrawForm: React.FC<WithdrawFormProps> = ({ account, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      const res = await api.accounts.withdraw(account.id, amount);
      const msg = res.message || `₹${parsed.toLocaleString('en-IN')} withdrawn from your account.`;
      setSuccessMsg(`Withdrawal successful. ${msg}`);
      setAmount('');
  ...
  return (
    <form id="withdraw-form" onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            disabled={isLoading}
            className="w-full pl-8 pr-4 py-2.5 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 font-mono text-slate-900 bg-white"
            required
          />
        </div>
      </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg bg-slate-800 hover:bg-slate-900 text-white font-medium shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <ArrowUpCircle className="w-4 h-4" />
        <span>{isLoading ? 'Withdrawing...' : 'Withdraw Money'}</span>
      </button>
    </form>
  );
};
